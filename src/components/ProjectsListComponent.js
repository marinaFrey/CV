...
const Tag = ({ tagName, isActive, selectTag}) => {
    ...
    return (
        <Badge color={color} className="p-2 m-1" pill onClick={selectTag} >{tagName}</Badge>
    );
}
...
const TimelineElement = ({ project }) => {
    ...
    return (
        <VerticalTimelineElement
            ...
            <Button color="primary" style={{ float: 'right' }} className="m-1" href={project.link}>Link to Project Page</Button>
            <Button color="dark" style={{ float: 'right' }} className="m-1"><Link to={"/projects/" + project.id}>More Details</Link></Button>
        </VerticalTimelineElement>
    );
}
